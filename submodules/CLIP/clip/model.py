...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, m: torch.Tensor = None):
        ...
        if m is not None:
            m = self.mask_pool(m.to(torch.float))[0, 0, ...].reshape(m.shape[0], -1).unsqueeze(-1)
            m = torch.ceil(m).to(x.dtype)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask=None):
        return self.visual(image.type(self.dtype), mask.type(self.dtype))
