...
RasterizeFeatureGaussiansCUDA(
  ...
  torch::Tensor out_color = torch::full({FEATURE_NUM_CHANNELS, H, W}, 0.0, float_opts);
  torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor radii =
      torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::featureforward(
        geomFunc, binningFunc, imgFunc, P, degree, M,
        background.contiguous().data<float>(), W, H,
        means3D.contiguous().data<float>(), sh.contiguous().data_ptr<float>(),
        colors.contiguous().data<float>(), opacity.contiguous().data<float>(),
...
// RasterizeFeatureGaussiansCUDA(
//     const torch::Tensor &background, const torch::Tensor &means3D,
//     const torch::Tensor &features, const torch::Tensor &opacity,
//     const torch::Tensor &scales, const torch::Tensor &rotations,
//     const float scale_modifier, const torch::Tensor &cov3D_precomp,
//     const torch::Tensor &viewmatrix, const torch::Tensor &projmatrix,
//     const float tan_fovx, const float tan_fovy, const int image_height,
...
//   torch::Tensor out_feature = torch::full({FEATURE_NUM_CHANNELS, H, W}, 0.0, float_opts);
//   torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts);
//   torch::Tensor radii =
//       torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
...
//   torch::Device device(torch::kCUDA);
//   torch::TensorOptions options(torch::kByte);
//   torch::Tensor geomfeatureBuffer = torch::empty({0}, options.device(device));
//   torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
//   torch::Tensor featureBuffer = torch::empty({0}, options.device(device));
//   std::function<char *(size_t)> geomfeatureFunc = resizeFunctional(geomfeatureBuffer);
//   std::function<char *(size_t)> binningFunc = resizeFunctional(binningBuffer);
//   std::function<char *(size_t)> featureFunc = resizeFunctional(featureBuffer);
...
//     rendered = CudaRasterizer::Rasterizer::featureforward(
//         geomfeatureFunc, binningFunc, featureFunc, P, degree, M,
//         background.contiguous().data<float>(), W, H,
//         means3D.contiguous().data<float>(), sh.contiguous().data_ptr<float>(),
//         features.contiguous().data<float>(), opacity.contiguous().data<float>(),
//         scales.contiguous().data_ptr<float>(), scale_modifier,
//         rotations.contiguous().data_ptr<float>(),
//         cov3D_precomp.contiguous().data<float>(),
//         viewmatrix.contiguous().data<float>(),
...
//         debug);
//   }
//   return std::make_tuple(rendered, out_feature, out_depth, radii, geomfeatureBuffer,
//                          binningBuffer, featureBuffer);
// }
...
RasterizeFeatureGaussiansBackwardCUDA(
  ...
  torch::Tensor dL_dmeans3D = torch::zeros({P, 3}, means3D.options());
  torch::Tensor dL_dmeans2D = torch::zeros({P, 3}, means3D.options());
  torch::Tensor dL_dcolors = torch::zeros({P, FEATURE_NUM_CHANNELS}, means3D.options());
  torch::Tensor dL_dconic = torch::zeros({P, 2, 2}, means3D.options());
  torch::Tensor dL_dopacity = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dcov3D = torch::zeros({P, 6}, means3D.options());
  torch::Tensor dL_dsh = torch::zeros({P, M, 3}, means3D.options());
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::featurebackward(
        P, degree, M, R, background.contiguous().data<float>(), W, H,
        means3D.contiguous().data<float>(), sh.contiguous().data<float>(),
        colors.contiguous().data<float>(), scales.data_ptr<float>(),
        scale_modifier, rotations.data_ptr<float>(),
...
// RasterizeFeatureGaussiansBackwardCUDA(
//     const torch::Tensor &background, const torch::Tensor &means3D,
//     const torch::Tensor &radii, const torch::Tensor &features,
//     const torch::Tensor &scales, const torch::Tensor &rotations,
//     const float scale_modifier, const torch::Tensor &cov3D_precomp,
//     const torch::Tensor &viewmatrix, const torch::Tensor &projmatrix,
//     const float tan_fovx, const float tan_fovy,
//     const torch::Tensor &dL_dout_color, const torch::Tensor &sh,
//     const int degree, const torch::Tensor &campos,
//     const torch::Tensor &geomfeatureBuffer, const int R,
//     const torch::Tensor &binningBuffer, const torch::Tensor &featureBuffer,
//     const bool debug) {
//   const int P = means3D.size(0);
//   const int H = dL_dout_color.size(1);
//   const int W = dL_dout_color.size(2);
...
//   torch::Tensor dL_dmeans3D = torch::zeros({P, 3}, means3D.options());
//   torch::Tensor dL_dmeans2D = torch::zeros({P, 3}, means3D.options());
//   torch::Tensor dL_dfeatures = torch::zeros({P, FEATURE_NUM_CHANNELS}, means3D.options());
//   torch::Tensor dL_dconic = torch::zeros({P, 2, 2}, means3D.options());
//   torch::Tensor dL_dopacity = torch::zeros({P, 1}, means3D.options());
//   torch::Tensor dL_dcov3D = torch::zeros({P, 6}, means3D.options());
//   torch::Tensor dL_dsh = torch::zeros({P, M, 3}, means3D.options());
...
//         P, degree, M, R, background.contiguous().data<float>(), W, H,
//         means3D.contiguous().data<float>(), sh.contiguous().data<float>(),
//         features.contiguous().data<float>(), scales.data_ptr<float>(),
//         scale_modifier, rotations.data_ptr<float>(),
//         cov3D_precomp.contiguous().data<float>(),
//         viewmatrix.contiguous().data<float>(),
//         projmatrix.contiguous().data<float>(),
//         campos.contiguous().data<float>(), tan_fovx, tan_fovy,
//         radii.contiguous().data<int>(),
//         reinterpret_cast<char *>(geomfeatureBuffer.contiguous().data_ptr()),
//         reinterpret_cast<char *>(binningBuffer.contiguous().data_ptr()),
//         reinterpret_cast<char *>(featureBuffer.contiguous().data_ptr()),
//         dL_dout_color.contiguous().data<float>(),
//         dL_dmeans2D.contiguous().data<float>(),
//         dL_dconic.contiguous().data<float>(),
//         dL_dopacity.contiguous().data<float>(),
...
//   return std::make_tuple(dL_dmeans2D, dL_dfeatures, dL_dopacity, dL_dmeans3D,
//                          dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
// }
