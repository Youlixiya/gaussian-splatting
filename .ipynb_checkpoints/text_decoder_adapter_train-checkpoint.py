...
class CLIPFeatureSemTokenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sem_token = torch.from_numpy(feature['sem_token'])
        return clip_feature, sem_token
...
def main(args):
    ...
    # val_dirs = ['sa_000020']
    train_dataset = CLIPFeatureSemTokenDataset(args.dataset_path)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # training
        model.train()
        for batch_idx, (clip_features, sem_tokens) in enumerate(train_loader):
            total_iters += 1
            
            clip_features = clip_features.cuda(args.local_rank).float()
            sem_tokens = sem_tokens.cuda(args.local_rank).float()
            optimizer.zero_grad()
            pred_sem_tokens = model(clip_features)
            loss = nn.functional.mse_loss(pred_sem_tokens, sem_tokens)
            loss.backward()
            optimizer.step()
