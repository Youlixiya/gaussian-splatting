...
from grounded_sam.LightHQSAM.setup_light_hqsam import setup_model
...
HQSAM_CHECKPOINT_PATH = "Grounded-SAM/weights/sam_hq_vit_tiny.pth"

class GroundLightHQSAM:
    def __init__(self,
                 grounding_dino_config_path=GROUNDING_DINO_CONFIG_PATH,
                 grounfing_dino_ckpt_path=GROUNDING_DINO_CHECKPOINT_PATH,
                 hqsam_ckpt_path=HQSAM_CHECKPOINT_PATH,
                 device='cuda'):
        ...
        self.hqsam_ckpt_path = hqsam_ckpt_path
        checkpoint = torch.load(hqsam_ckpt_path)
    ...
    def __call__(self,
        ...
        if pil:
            return PIL.Image.fromarray(annotated_box[:, :, ::-1]), PIL.Image.fromarray(annotated_mask[:, :, ::-1])
        else:
            return annotated_box, annotated_mask
